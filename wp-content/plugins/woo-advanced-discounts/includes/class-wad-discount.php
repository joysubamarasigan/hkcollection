...
//        $rule_tpl = $this->get_rule_tpl($conditions);
    ...
    function apply_product_discounts($cart) {
        //Désactivé car applique 2x la réduction au panier pour celle concernant le moyen de payment par example
...
//                $list = $discount_obj->products_list;
                ...
                $discount_obj->apply_discount_on($cart, $list_products);
            }
        }
    }
...
//	endif;
    ...
    function get_cart_total_discount_row() {
        $to_widthdraw = $this->get_total_cart_discount();
        if (!$to_widthdraw)
            return;
        ?>
        <tr class="order-discount">
            <th><?php
                _e('Total cart discount', 'wad');
                ?>
                <span data-tooltip-title='<?php _e("Only includes the discounts applied on the order itself, not on the individual products.", "wad"); ?>'>
                    <img src="<?php echo WAD_URL . "/public/images/help.png"; ?>">
                </span>
            </th>
            <td><?php
                $to_widthdraw_html = '<span class="wad-discount">-' . wc_price($to_widthdraw) . '</span>';
                echo $to_widthdraw_html;
                ?></td>
        </tr>
        <?php
    }
...
//            array_push($_SESSION["active_discounts"], $this->id);
    ...
    private function get_cart_products_count($by_products = false) {
        ...
        {
            foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item_data) {
    //            array_push($count, $cart_item_data["product_id"]);
                //We add the variations too in case the customer is checking against a variation
                if (isset($cart_item_data["variation_id"]) && !empty($cart_item_data["variation_id"])) {
                    $count[$cart_item_data["variation_id"]] = $cart_item_data["quantity"];
                    //array_push($count, $cart_item_data["variation_id"]);
    //                var_dump($cart_item_data["variation_id"]."=>".$cart_item_data["quantity"]);
                } else {
    //                var_dump($cart_item_data["product_id"]."=>".$cart_item_data["quantity"]);
...
//        global $woocommerce;
        switch ($condition) {
            ...
            case "customer"://If Customer is
                ...
                break;
            case "previous-order-count"://If Previous orders count
                $evaluable_condition = count($this->get_customer_orders());
                break;
    ...
    function get_active_discounts($group_by_types = false) {
        ...
        foreach ($discounts as $discount) {
            ...
            if (
                    ...
                    $this->is_in_valid_period($metas["start-date"], $metas["end-date"], $metas["period"], $metas["period-type"])
            ) {
                if ($group_by_types) {
    ...
    private function is_in_valid_period($start, $end, $period, $period_type) {
        if (empty($period)) {
            return false;
        }
    ...
    public function get_sale_price($sale_price, $product) {
        ...
        $pid = $this->get_product_id_to_use($product);

//        $price_to_apply_discount_on = $product->sale_price;
        if (empty($sale_price))
...
//                echo "From WAD: $pid => $regular_price<br>";
                //We save the discount in the session to use it later when completing the payment
                if (!in_array($discount_id, $_SESSION["active_discounts"]) && wad_is_checkout()) {
                    array_push($_SESSION["active_discounts"], $discount_id);
                }
...
//                    echo "<hr><hr>";
//                    var_dump($woocommerce->cart->subtotal_ex_tax);
//                    echo "<hr><hr>";
//                }
        }
...
//        var_dump($regular_price);
     ...
     * @return int
     */
    private function get_product_id_to_use($product) {
...
//        var_dump($quantity_pricing);
    ...
    function get_total_cart_discounts($total_discount, $order) {
        $stored_order_total = $order->get_total();
        $total_taxes = $order->get_total_tax();
        $total_shipping = $order->get_total_shipping();
        $items = $order->get_items();
        $calculated_order_total = 0;
        foreach ($items as $item) {
            $calculated_order_total+=$item["item_meta"]["_line_total"][0];
        }
        $total_fees = 0;
        if ($fees = $order->get_fees()) {
            foreach ($fees as $id => $fee) {

                $total_fees+=$fee['line_total'] + $fee['line_tax'];
            }
        }

        $new_total_discount = $calculated_order_total - $stored_order_total;

        return $new_total_discount + $total_taxes + $total_shipping + $total_fees;
    }
    ...
    function get_variaion_prices($prices, $product) {
        foreach ($prices["regular_price"] as $variation_id => $variation_price) {
            $variation = wc_get_product($variation_id);
            $prices["regular_price"][$variation_id] = $this->get_regular_price($variation_price, $variation);
//                    var_dump($variation_id);
//                    var_dump($prices["regular_price"][$variation_id]);
//                    var_dump($prices["sale_price"][$variation_id]);
...
//                echo $prices["regular_price"][1977]." vs ".$prices["sale_price"][1977];
//                echo "<hr>";
        return $prices;
    }
