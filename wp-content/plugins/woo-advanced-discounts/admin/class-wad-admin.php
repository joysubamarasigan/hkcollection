...
//        add_submenu_page($parent_slug, __('Get Started', 'wad'), __('Get Started', 'wad'), 'manage_product_terms', 'wad-about', array($this, "get_about_page"));
    ...
    function get_wad_pro_features_page()
    {
        ...
        <div id="wad-pro-features">
            <div class="o-wrap">
                <h2 style="text-align: center;"><a href="http://www.orionorigin.com/plugins/woocommerce-advanced-discounts/" target="blank">Need more features? Let's go pro!</a></h2>
                <div style="text-align: center;">
                    <a href="http://www.orionorigin.com/plugins/woocommerce-advanced-discounts/" target="blank">
                    <img src="<?php echo WAD_URL."admin/images/go-pro.png"?>">
                    </a>
                </div>
            </div>
        </div>
    ...
    function get_ad_messages()
    {
        global $pagenow;
        $messages=array(
            "<strong>Free gifts</strong>: create a \"Buy one, get one for free\" kind of discount",
            "<strong>Discounts by shipping country</strong>: apply a discount based on the shipping country.",
            "<strong>Discounts by billing country</strong>: apply a discount based on the billing country.",
            "<strong>Discounts by payment gateway</strong>: apply a discount if the customers checks out with a specific payment gateway.",
            "<strong>Usage limit</strong>: limits the number of customers who can use a discount.",
            "<strong>Periodic discounts</strong>: automatically enable a discount periodically.",
            "<strong>Social networks</strong>: apply a discounts if the customer is following you on social networks.",
            "<strong>Groups based discounts</strong>: apply a discount is the customer belong to a specific group.",
            "<strong>Newsletters based discounts</strong>: offer a discount if the customer subscribed to your newsletters.",
            "<strong>Taxes inclusion</strong>: apply discounts on subtotal with or without the taxes.",
            "<strong>Specific users discounts</strong>: apply discounts for specific(s) customer(s).",
        );
        ...
        )
        {
            echo '<div class="wad-info">
               <p>'.$messages[$random_message_key].' <a href="http://www.orionorigin.com/plugins/woocommerce-advanced-discounts/" class="button"  target="_blank">Click here to unlock</a></p>
            </div>';
        }
...
//        var_dump($ignore_notices);
        $dismiss_transient=get_transient( 'wad_notice_dismiss' );
//        var_dump($dismiss_transient);
        if($ignore_notices||$dismiss_transient!==false)
            return;
//        $admin_notices_option = get_option( 'wad_admin_notice', array() );
//        var_dump($admin_notices_option);
//        if ( ! isset( $admin_notices_option[ 'disable_admin_notices' ] ) || ( isset( $admin_notices_option[ 'disable_admin_notices' ] ) && $admin_notices_option[ 'disable_admin_notices' ] == 0 ) ){
...
//                $admin_notices_option = get_option( 'wad_admin_notice', array() );
//                $admin_notices_option[ $_GET[ 'wad_admin_notice_ignore' ] ][ 'dismissed' ] = 1;
...
//                 update_option( 'wad_admin_notice', "1" );
//                $admin_notices_option = get_option( 'wad_admin_notice', array() );
//
...
//
//                $admin_notices_option[ $_GET[ 'wad_admin_notice_temp_ignore' ] ][ 'start' ] = $new_start;
//                $admin_notices_option[ $_GET[ 'wad_admin_notice_temp_ignore' ] ][ 'dismissed' ] = 0;
//                update_option( 'wad_admin_notice', $admin_notices_option );
