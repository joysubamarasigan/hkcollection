	...
	/**jQuery(document).ready(function (){
		...
		jQuery('#billing_state').on('change', function(){
			if(jQuery(this).val() != '00'){
				jQuery('input.shipping_method').parent().removeClass('hide_shipping_class');
				jQuery('input[value="main"]').parent().addClass('hide_shipping_class');
			}
		});
    });**/
</script>
	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( 1 < count( $available_methods ) ) : ?>
			<ul id="shipping_method">
				...
				$customer_state = get_user_meta(get_current_user_id(), 'billing_state', true);
				...
				foreach ( $available_methods as $method ) :
					...
					if(checked( $method->id, $chosen_method, false) == ''){
						...
						$hide_shipping_class = 'hide_shipping_class';

					}
				?>

				<li <?php echo 'class="' . $hide_shipping_class . '"'; ?>>
